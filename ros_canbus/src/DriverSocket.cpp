...
#include <ros_driver_base/file_guard.hpp>

using namespace canbus;
using ros_driver_base::UnixError;
using ros_driver_base::TimeoutError;
using ros_driver_base::Timeout;
...
bool DriverSocket::open(std::string const& path)
{
    ...
    ros_driver_base::FileGuard guard(fd);
...
bool DriverSocket::checkInput(Timeout timeout)
{
    ...
    while(true) {
        ...
        if (res == -1 && errno != EAGAIN) 
            throw ros_driver_base::UnixError("read(): error in recvmsg()");
        if (res >= 0) {
...
Message DriverSocket::read()
{
    ...
    while (rx_queue.empty()) {
        if (!checkInput(timeout))
            throw ros_driver_base::TimeoutError(ros_driver_base::TimeoutError::PACKET, "read(): timeout");
    }
...
bool DriverSocket::readCanMsg(Message &result) {
    Timeout timeout(m_read_timeout);
    while (rx_queue.empty()) {
        if (!checkInput(timeout))
            return false;
