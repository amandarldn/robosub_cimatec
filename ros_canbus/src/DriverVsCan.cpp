#include <ros_canbus/DriverVsCan.hpp>
...
bool DriverVsCan::reset()
{
    ...
    timestampBase = base::Time::fromSec(0);
    return true;
}
...
bool DriverVsCan::checkForMessages(ros_driver_base::Timeout& timeout)
{
#define BUFFER_SIZE 100
    ...
    while(rx_cnt < 1)
    {
        ...
        if((ret = VSCAN_Read(handle, buffer, BUFFER_SIZE, &read_cnt) != VSCAN_ERR_OK))
            throw ros_driver_base::UnixError("read(): error in reading can messages");
...
{
    ros_driver_base::Timeout timeout(m_read_timeout);

    if(!checkForMessages(timeout))
        throw ros_driver_base::TimeoutError(
                ros_driver_base::TimeoutError::PACKET, "read(): timeout");
...
void DriverVsCan::write(Message const& msg)
{
    ...
    if(VSCAN_Write(handle, &out, 1, &out_cnt) != VSCAN_ERR_OK)
        throw ros_driver_base::UnixError("write(): error during write");
}

int DriverVsCan::getPendingMessagesCount()
{
    ros_driver_base::Timeout t(0);
    checkForMessages(t);
...
{
    return ros_driver_base::Driver::INVALID_FD;
}
