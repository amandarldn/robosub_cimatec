...
DriverNetGateway::DriverNetGateway()
    : ros_driver_base::Driver(sizeof(CanFrame))
    , mErrorCounter(0)
    , mError(false)
{
    m_read_timeout = std::chrono::milliseconds(DriverNetGateway::DEFAULT_TIMEOUT);
    m_write_timeout = std::chrono::milliseconds(DriverNetGateway::DEFAULT_TIMEOUT);
}
...
{
    std::chrono::milliseconds timeout = std::chrono::duration_cast<std::chrono::milliseconds>(ros_driver_base::Driver::getReadTimeout());
    return static_cast<uint32_t>(timeout.count());
}
