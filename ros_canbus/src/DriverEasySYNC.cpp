...
DriverEasySYNC::DriverEasySYNC(int queue_size)
    : ros_driver_base::Driver(MAX_PACKET_SIZE)
{
...
{
    ros_driver_base::Driver::setWriteTimeout(std::chrono::milliseconds(timeout));
}

uint32_t DriverEasySYNC::getWriteTimeout() const
{
    // return ros_driver_base::Driver::getWriteTimeout().toMilliseconds();
    std::chrono::milliseconds timeout = std::chrono::duration_cast<std::chrono::milliseconds>(ros_driver_base::Driver::getWriteTimeout());
    return static_cast<uint32_t>(timeout.count());
}

void DriverEasySYNC::setReadTimeout(uint32_t timeout)
{
    // ros_driver_base::Driver::setReadTimeout(base::Time::fromMilliseconds(timeout));
    ros_driver_base::Driver::setReadTimeout(std::chrono::milliseconds(timeout));
}

uint32_t DriverEasySYNC::getReadTimeout() const
{
    std::chrono::duration<double> readTimeoutDuration = ros_driver_base::Driver::getReadTimeout();
    return static_cast<uint32_t>(std::chrono::duration_cast<std::chrono::milliseconds>(readTimeoutDuration).count());
}
...
{
    return ros_driver_base::Driver::getFileDescriptor();
}

bool DriverEasySYNC::isValid() const
{
    return ros_driver_base::Driver::isValid();
}

void DriverEasySYNC::close()
{
    processSimpleCommand("C\r", 2);
    ros_driver_base::Driver::close();
}
